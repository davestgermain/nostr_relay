...
class DBTests(BaseTestsWithStorage):
    ...
    async def test_expiration_tag(self):
        ...
        assert (
            await self.storage.get_event(expiring_event["id"])
        ).id == expiring_event["id"]
        assert (
            await self.storage.get_event(distant_future_event["id"])
    ...
    async def test_ephemeral_event(self):
        ...
        assert (
            await self.storage.get_event(ephemeral_event["id"])
        ).id == "2696df86ce47142b7d272408e222b7a9fc4b2cc3a428bf2debf5d730ae2f42c7"
        ...
        async with self.conductor.simulate_ws("/") as ws:
            ...
            await ws.send_json(
                [
                    ...
                    {
                        "#p": [
                            "44c7a871ce6224d8d274b494f8f68827cb966e3aaba723a14db8dd22e0542e7d"
                        ]
                    },
                ]
            )
...
class AuthTests(APITests):
    async def asyncSetUp(self):
        Config.authentication = {
            ...
            "throttle": {"unauthenticated": 1, "t": 5.0},
        }
        await super().asyncSetUp()
        self.readonly = (
        ...
        self.throttled = (
            "79612c2c2eae6f89d4884b6a518ca386c87f94e0ec9f417148afc201ddee6d38",
            "eb669db891c0a506ec3d5314c724d1684f36332533643c83f79f8ea22c059d99",
            "tw",
        )
        ...
        async with self.conductor.simulate_ws("/") as ws:
            ...
            await ws.send_json(
                [
                    "AUTH",
                    self.make_auth_event(
                        self.throttled[0], self.throttled[1], challenge=challenge
                    ),
                ]
            )
            with self.assertRaises(asyncio.TimeoutError):
            ...
            await ws.send_json(
                [
                    "AUTH",
                    self.make_auth_event(
                        self.readwrite[0], self.readwrite[1], challenge=challenge
                    ),
                ]
            )
            async with asyncio.timeout(0.5):
